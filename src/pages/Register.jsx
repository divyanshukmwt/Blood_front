...
const Register = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (res.status === 201) { setUser(res.data.user || res.data); navigate("/login"); toast.success("Account created! Please sign in."); }
  ...
  return (
    <div style={{ minHeight:'100vh',background:'var(--ash)',display:'flex',alignItems:'center',justifyContent:'center',padding:'24px' }}>
      <div style={{ width:'100%',maxWidth:'520px',background:'white',borderRadius:'24px',padding:'56px 48px',boxShadow:'var(--card-shadow)' }} className="reg-card">
        ...
        <h1 style={{ fontFamily:'Syne, sans-serif',fontWeight:800,fontSize:'1.75rem',letterSpacing:'-0.02em',marginBottom:'8px' }}>Create account</h1>
        ...
        <form onSubmit={handleSubmit(onSubmit)} style={{ display:'flex',flexDirection:'column',gap:'18px' }}>
          <div>
            ...
            <input {...register("name",{ required:"Name is required",minLength:{ value:6,message:"Min 6 characters" },pattern:{ value:/^[A-Za-z\s]+$/,message:"Letters only" },validate:v=>!AbandonWord.some(b=>v.toLowerCase().includes(b))||"Contains restricted words" })}
              onBlur={e=>setValue("name",e.target.value.trim(),{ shouldValidate:true })}
              type="text" placeholder="Full name" style={inputStyle(errors.name)}
          ...
          <div>
            ...
            <input {...register("email",{ required:"Email is required",pattern:{ value:/^[^\s@]+@[^\s@]+\.[^\s@]+$/,message:"Invalid email" } })}
              type="email" placeholder="you@example.com" style={inputStyle(errors.email)}
              onFocus={e=>e.target.style.borderColor='var(--crimson)'}
          ...
          <div>
            ...
            <select {...register("bloodgroup",{ required:"Select blood group",validate:v=>v!=="default"||"Select a valid blood group" })} defaultValue="default"
          ...
          <button type="submit" disabled={loading} style={{ padding:'14px',borderRadius:'10px',background:loading?'var(--muted)':'var(--crimson)',color:'white',border:'none',cursor:loading?'not-allowed':'pointer',fontFamily:'DM Sans, sans-serif',fontWeight:700,fontSize:'0.95rem',marginTop:'8px',transition:'all 0.2s',display:'flex',alignItems:'center',justifyContent:'center',gap:'8px' }}>
            {loading?'Creating account…':<><span>Create account</span><ArrowRight size={16}/></>}
          </button>
        </form>
      </div>
      <style>{`@media(max-width:480px){.reg-card{padding:36px 24px!important;}}`}</style>
    </div>
  );
};
