    ...
    <footer style={{ background:'var(--ink)',color:'var(--white)',padding:'64px 0 32px' }}>
      <div style={{ maxWidth:'1200px',margin:'0 auto',padding:'0 40px' }}>
        <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fit, minmax(200px, 1fr))',gap:'48px',paddingBottom:'48px',borderBottom:'1px solid rgba(255,255,255,0.1)' }}>
          <div>
            <div style={{ display:'flex',alignItems:'center',gap:'10px',marginBottom:'16px' }}>
              ...
              <span style={{ fontFamily:'Syne, sans-serif',fontWeight:800,fontSize:'1.2rem' }}>Red<span style={{ color:'var(--crimson-light)' }}>Hope</span></span>
            </div>
            <p style={{ color:'rgba(255,255,255,0.5)',fontSize:'0.875rem',lineHeight:1.7,maxWidth:240 }}>Every drop saves a life. Together we build a healthier India, one donation at a time.</p>
          </div>

          {[['Services',links.service],['Help',links.helps],['Legal',links.legal]].map(([title,items])=>(
            <div key={title}>
              <p style={{ fontSize:'0.7rem',fontWeight:700,letterSpacing:'0.1em',textTransform:'uppercase',color:'rgba(255,255,255,0.4)',marginBottom:'20px' }}>{title}</p>
              {items.map((l,i)=>(
                <a key={i} href={l.href} download={l.download?l.name:undefined}
                  style={{ display:'block',color:'rgba(255,255,255,0.7)',fontSize:'0.9rem',textDecoration:'none',marginBottom:'12px',transition:'color 0.2s' }}
        ...
        <div style={{ paddingTop:'24px',display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:'12px' }}>
          <p style={{ color:'rgba(255,255,255,0.35)',fontSize:'0.8rem' }}>© 2024 RedHope. All rights reserved.</p>
          <p style={{ color:'rgba(255,255,255,0.35)',fontSize:'0.8rem' }}>Made with care for India</p>
        </div>
      </div>
    </footer>
  );
