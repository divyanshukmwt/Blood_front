...
const AdminProfilePic = ({ email, fn }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    formData.append("profilepic",file);formData.append("email",email);
    try{
      const res=await Axios.post("/admin/adminPic",formData,{headers:{"Content-Type":"multipart/form-data"},withCredentials:true});
      fn(false);setAdmin(res.data.Useradmin);toast.success("Profile photo updated!");
    ...
    <div style={{ position:'fixed',inset:0,zIndex:1000,background:'rgba(15,13,12,0.7)',backdropFilter:'blur(8px)',display:'flex',alignItems:'center',justifyContent:'center',padding:'24px' }}
      ...
      <div style={{ background:'white',borderRadius:'24px',padding:'40px',width:'100%',maxWidth:'400px',animation:'fade-up 0.3s cubic-bezier(0.22,1,0.36,1)',position:'relative' }}>
        ...
        <form onSubmit={handleSubmit} style={{ display:'flex',flexDirection:'column',gap:'16px' }}>
          ...
          <button type="submit" disabled={!file||uploading} style={{ padding:'13px',borderRadius:'10px',background:!file||uploading?'var(--muted)':'var(--crimson)',color:'white',border:'none',cursor:!file||uploading?'not-allowed':'pointer',fontFamily:'DM Sans, sans-serif',fontWeight:700,fontSize:'0.95rem',display:'flex',alignItems:'center',justifyContent:'center',gap:'8px' }}>
